...
data class BluewhalePacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = BluewhalePacket(
            version = version,
            type = type,
            senderID = senderID,
            ...
            route = route,
            ttl = com.bluewhale.android.util.AppConstants.SYNC_TTL_HOPS // Use fixed TTL=0 for signing to ensure relay compatibility
        )
        return BinaryProtocol.encode(unsignedPacket)
    }

    companion object {
        fun fromBinaryData(data: ByteArray): BluewhalePacket? {
...
object BinaryProtocol {
    ...
    fun decode(data: ByteArray): BluewhalePacket? {
     ...
     */
    private fun decodeCore(raw: ByteArray): BluewhalePacket? {
        try {
