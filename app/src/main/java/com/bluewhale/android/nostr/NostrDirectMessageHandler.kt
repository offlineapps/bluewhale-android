...
import com.bluewhale.android.services.SeenMessageStore
import com.bluewhale.android.ui.ChatState
import com.bluewhale.android.ui.MeshDelegateHandler
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.bluewhale.android.ui.DataManager
) {
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch(Dispatchers.Default) {
            try {
                ...
                if (!content.startsWith("bluewhale1:")) return@launch

                val base64Content = content.removePrefix("bluewhale1:")
                val packetData = base64URLDecode(base64Content) ?: return@launch
                val packet = BluewhalePacket.fromBinaryData(packetData) ?: return@launch

                if (packet.type != com.bluewhale.android.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                com.bluewhale.android.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
    ...
    private suspend fun processNoisePayload(
        ...
        when (payload.type) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                val file = BluewhaleFilePacket.decode(payload.data)
                if (file != null) {
                    val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                    val savedPath = com.bluewhale.android.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = BluewhaleMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
            ...
            NoisePayloadType.PEER_IDENTITY -> Unit
        }
    }
