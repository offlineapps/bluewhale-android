package com.bluewhale.android.ui.debug
        ...
        if (isPresented) {
            ...
            while (true) {
                ...
                val devices = entries.map { (address, isClient, rssi) ->
                    ...
                    com.bluewhale.android.ui.debug.ConnectedDevice(
            ...
            item {
                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            Icon(Icons.Filled.SettingsEthernet, contentDescription = null, tint = colorScheme.primary)
            ...
            item {
                Surface(shape = RoundedCornerShape(12.dp), color = colorScheme.surfaceVariant.copy(alpha = 0.2f)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        ...
                        val p = remember(gcsFpr) { com.bluewhale.android.sync.GCSFilter.deriveP(gcsFpr / 100.0) }
                        val nmax = remember(gcsFpr, gcsMaxBytes) { com.bluewhale.android.sync.GCSFilter.estimateMaxElementsForSize(gcsMaxBytes, p) }
                            ...
                            scanResults.forEach { res ->
                                Surface(shape = RoundedCornerShape(8.dp), color = colorScheme.surface.copy(alpha = 0.6f)) {
                                    Row(Modifier.fillMaxWidth().padding(10.dp), verticalAlignment = Alignment.CenterVertically) {
                                        ...
                                        Text(stringResource(R.string.debug_connect), color = colorScheme.primary, fontFamily = FontFamily.Monospace, modifier = Modifier.clickable {
