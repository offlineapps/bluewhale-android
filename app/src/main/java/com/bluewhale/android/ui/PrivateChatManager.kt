...
import com.bluewhale.android.mesh.PeerFingerprintManager
...
class PrivateChatManager(
    ...
    fun handleIncomingPrivateMessage(message: BluewhaleMessage) {
        handleIncomingPrivateMessage(message, suppressUnread = false)
    }

    fun handleIncomingPrivateMessage(message: BluewhaleMessage, suppressUnread: Boolean) {
    ...
    fun sendReadReceiptsForPeer(peerID: String, meshService: BluetoothMeshService) {
        ...
        val chats = try { state.getPrivateChatsValue() } catch (_: Exception) { emptyMap<String, List<BluewhaleMessage>>() }
    ...
    private fun consolidateNostrTempConversationIfNeeded(targetPeerID: String) {
        ...
        try {
            ...
            val npub = com.bluewhale.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(noiseKeyBytes)
            if (npub != null) {
                // Normalize to hex to match how we formed temp keys (nostr_<pub16>)
                val (hrp, data) = com.bluewhale.android.nostr.Bech32.decode(npub)
                if (hrp == "npub") {
