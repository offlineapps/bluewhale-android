...
class BluetoothConnectionManager(
    ...
    private val componentDelegate = object : BluetoothConnectionManagerDelegate {
        override fun onPacketReceived(packet: BluewhalePacket, peerID: String, device: BluetoothDevice?) {
     ...
     */
    fun sendPacketToPeer(peerID: String, packet: BluewhalePacket): Boolean {
        if (!isActive) return false
    ...
    override fun onPowerModeChanged(newMode: PowerManager.PowerMode) {
        ...
        connectionScope.launch {
            ...
            val serverEnabled = try { com.bluewhale.android.ui.debug.DebugSettingsManager.getInstance().gattServerEnabled.value } catch (_: Exception) { true }
            if (serverEnabled) {
            ...
            if (wasUsingDutyCycle != nowUsingDutyCycle) {
                ...
                val clientEnabled = try { com.bluewhale.android.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true }
                if (clientEnabled) {
...
interface BluetoothConnectionManagerDelegate {
    fun onPacketReceived(packet: BluewhalePacket, peerID: String, device: BluetoothDevice?)
