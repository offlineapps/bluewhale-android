...
class SecurityManager(private val encryptionService: EncryptionService, private val myPeerID: String) {
    ...
    companion object {
        ...
        private const val MESSAGE_TIMEOUT = com.bluewhale.android.util.AppConstants.Security.MESSAGE_TIMEOUT_MS // 5 minutes (same as iOS)
     ...
     */
    fun validatePacket(packet: BluewhalePacket, peerID: String): Boolean {
    ...
    private fun verifyPacketSignature(packet: BluewhalePacket, peerID: String): Boolean {
        try {
            ...
            if (MessageType.fromValue(packet.type) == MessageType.ANNOUNCE) {
                ...
                try {
                    val announcement = com.bluewhale.android.model.IdentityAnnouncement.decode(packet.payload)
