...
import com.bluewhale.android.sync.GossipSyncManager
...
class BluetoothMeshService(private val context: Context) {
    ...
    companion object {
        ...
        private val MAX_TTL: UByte = com.bluewhale.android.util.AppConstants.MESSAGE_TTL_HOPS
    }
    ...
    init {
        ...
        gossipSyncManager = GossipSyncManager(
            ...
            configProvider = object : GossipSyncManager.ConfigProvider {
                override fun seenCapacity(): Int = try {
                    com.bluewhale.android.ui.debug.DebugPreferenceManager.getSeenPacketCapacity(500)
                } catch (_: Exception) { 500 }

                override fun gcsMaxBytes(): Int = try {
                    com.bluewhale.android.ui.debug.DebugPreferenceManager.getGcsMaxFilterBytes(400)
                } catch (_: Exception) { 400 }

                override fun gcsTargetFpr(): Double = try {
                    com.bluewhale.android.ui.debug.DebugPreferenceManager.getGcsFprPercent(1.0) / 100.0
        ...
        gossipSyncManager.delegate = object : GossipSyncManager.Delegate {
            ...
            override fun sendPacketToPeer(peerID: String, packet: BluewhalePacket) {
                connectionManager.sendPacketToPeer(peerID, packet)
            }
            override fun signPacketForBroadcast(packet: BluewhalePacket): BluewhalePacket {
    ...
    private fun setupDelegates() {
        ...
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                ...
                try { com.bluewhale.android.services.AppStateStore.setPeers(peerIDs) } catch (_: Exception) { }
            ...
            override fun onPeerRemoved(peerID: String) {
                ...
                try { com.bluewhale.android.services.meshgraph.MeshGraphService.getInstance().removePeer(peerID) } catch (_: Exception) { }
        ...
        securityManager.delegate = object : SecurityManagerDelegate {
            ...
            override fun sendHandshakeResponse(peerID: String, response: ByteArray) {
                ...
                val responsePacket = BluewhalePacket(
                    version = 1u,
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun verifySignature(packet: BluewhalePacket, peerID: String): Boolean {
            ...
            override fun updatePeerIDBinding(newPeerID: String, nickname: String,
                ...
                try {
                    com.bluewhale.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(publicKey)?.let { npub ->
                        com.bluewhale.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(newPeerID, npub)
                    }
                } catch (_: Exception) { }
                ...
                try {
                    when {
                        message.isPrivate -> {
                            ...
                            if (peer.isNotEmpty()) com.bluewhale.android.services.AppStateStore.addPrivateMessage(peer, message)
                        }
                        message.channel != null -> {
                            com.bluewhale.android.services.AppStateStore.addChannelMessage(message.channel!!, message)
                        }
                        else -> {
                            com.bluewhale.android.services.AppStateStore.addPublicMessage(message)
                        }
                    }
                } catch (_: Exception) { }
                ...
                if (delegate == null && message.isPrivate) {
                    try {
                        ...
                        if (senderPeerID != null) {
                            ...
                            val preview = com.bluewhale.android.ui.NotificationTextUtils.buildPrivateMessagePreview(message)
        ...
        packetProcessor.delegate = object : PacketProcessorDelegate {
            ...
            override fun handleFragment(packet: BluewhalePacket): BluewhalePacket? {
        ...
        override fun onPacketReceived(packet: BluewhalePacket, peerID: String, device: android.bluetooth.BluetoothDevice?) {
            // Log incoming for debug graphs (do not double-count anywhere else)
            try {
                com.bluewhale.android.ui.debug.DebugSettingsManager.getInstance().logIncoming(
                    packet = packet,
                    fromPeerID = peerID,
                    fromNickname = null,
            ...
            override fun onDeviceConnected(device: android.bluetooth.BluetoothDevice) {
                ...
                try {
                    ...
                    com.bluewhale.android.ui.debug.DebugSettingsManager.getInstance()
     ...
     */
    fun sendFileBroadcast(file: com.bluewhale.android.model.BluewhaleFilePacket) {
        try {
     ...
     */
    fun sendFilePrivate(recipientPeerID: String, file: com.bluewhale.android.model.BluewhaleFilePacket) {
        try {
            ...
            serviceScope.launch {
                ...
                if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                    try {
                        ...
                        val noisePayload = com.bluewhale.android.model.NoisePayload(
                            type = com.bluewhale.android.model.NoisePayloadType.FILE_TRANSFER,
                            data = filePayload
                        )
                        ...
                        val packet = BluewhalePacket(
                            ...
                            payload = encrypted,
                            signature = null,
                            ttl = com.bluewhale.android.util.AppConstants.MESSAGE_TTL_HOPS
                        )
        ...
        serviceScope.launch {
            ...
            if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                try {
                    ...
                    val privateMessage = com.bluewhale.android.model.PrivateMessagePacket(
                    ...
                    val messagePayload = com.bluewhale.android.model.NoisePayload(
                        type = com.bluewhale.android.model.NoisePayloadType.PRIVATE_MESSAGE,
                        data = tlvData
                    )
    ...
    fun sendReadReceipt(messageID: String, recipientPeerID: String, readerNickname: String) {
        serviceScope.launch {
            ...
            val geo = runCatching { com.bluewhale.android.services.MessageRouter.tryGetInstance() }.getOrNull()
            val isGeoAlias = try {
                val map = com.bluewhale.android.nostr.GeohashAliasRegistry.snapshot()
                map.containsKey(recipientPeerID)
            } catch (_: Exception) { false }
            if (isGeoAlias && geo != null) {
                geo.sendReadReceipt(com.bluewhale.android.model.ReadReceipt(messageID), recipientPeerID)
                return@launch
            }

            try {
                // Avoid duplicate read receipts: check persistent store first
                val seenStore = try { com.bluewhale.android.services.SeenMessageStore.getInstance(context.applicationContext) } catch (_: Exception) { null }
                ...
                val readReceiptPayload = com.bluewhale.android.model.NoisePayload(
                    type = com.bluewhale.android.model.NoisePayloadType.READ_RECEIPT,
                ...
                val packet = BluewhalePacket(
                    ...
                    payload = encrypted,
                    signature = null,
                    ttl = com.bluewhale.android.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS messageTTL
                )
    ...
    fun sendBroadcastAnnounce() {
        ...
        serviceScope.launch {
            ...
            try {
                ...
                try {
                    com.bluewhale.android.services.meshgraph.MeshGraphService.getInstance()
     ...
     */
    fun getSessionState(peerID: String): com.bluewhale.android.noise.NoiseSession.NoiseSessionState {
        ...
        return try {
            ...
            val withRoute = try {
                ...
                if (rec != null && !rec.contentEquals(SpecialRecipients.BROADCAST)) {
                    ...
                    val path = com.bluewhale.android.services.meshgraph.RoutePlanner.shortestPath(myPeerID, dest)
...
interface BluetoothMeshDelegate {
    fun didReceiveMessage(message: BluewhaleMessage)
